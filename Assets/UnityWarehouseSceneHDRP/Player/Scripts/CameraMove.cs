...
namespace UnityWarehouseSceneHDRP
{
	public class CameraMove : MonoBehaviour
	{
        ...
        [ContextMenu("Disable Static On All Pallets")]
        private void DisableStaticOnAllPallets()
        {
            GameObject[] pallets = GameObject.FindGameObjectsWithTag("Pallet");

            int count = 0;

            foreach (GameObject pallet in pallets)
            {
                foreach (Transform t in pallet.GetComponentsInChildren<Transform>(true))
                {
            ...
            Debug.Log($"Disabled static on {count} objects.");
        }
        private void Update()
		{
			// Rotate
